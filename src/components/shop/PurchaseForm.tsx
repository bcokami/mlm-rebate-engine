...
import { FaSpinner, FaShoppingCart, FaShippingFast } from "react-icons/fa";
...
export default function PurchaseForm({ product, onSuccess, referralCode }: PurchaseFormProps) {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (selectedShippingMethod && selectedShippingMethod.code !== 'pickup' && !shippingAddress) {
      ...
      // Add shipping method details to the data
      if (selectedShippingMethod) {
        data.shippingMethodId = selectedShippingMethod.id;
        data.shippingDetails = shippingDetails;
        data.shippingAddress = shippingAddress;
        data.shippingFee = selectedShippingMethod.baseFee;
      }
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="mb-6">
          <label className="block text-sm font-medium text-gray-700 mb-2">
            <FaShippingFast className="inline mr-1" /> Shipping Method
          </label>
        ...
        <button
          type="submit"
          disabled={loading || !selectedPaymentMethod || !selectedShippingMethod}
